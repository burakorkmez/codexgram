...
export const begin = mutation({ args: { purpose: v.union(v.literal('post'), v.literal('avatar'), v.literal('story')), kind: mediaKind, width: v.number(), height: v.number(), duration: v.optional(v.number()) }, returns: v.id('uploads'), handler: async (ctx, args) => {
  const me = await requireProfile(ctx);
  if (![args.width, args.height].every(n => Number.isInteger(n) && n > 0 && n <= 30000)) throw new ConvexError('Invalid media dimensions.');
  if (args.kind === 'video' && (args.purpose !== 'post' || !args.duration || !Number.isFinite(args.duration) || args.duration > 30 || args.duration <= 0)) throw new ConvexError('Videos must be 30 seconds or shorter.');
...
export const forUpload = internalQuery({ args: { id: v.string(), tokenIdentifier: v.string() }, returns: v.union(schema.doc('uploads'), v.null()), handler: async (ctx, args) => {
  ...
  return owner?.tokenIdentifier === args.tokenIdentifier && !owner.deletionRequested ? upload : null;
}});
export const finish = internalMutation({ args: { id: v.id('uploads'), tokenIdentifier: v.string(), storageId: v.id('_storage'), duration: v.optional(v.number()) }, returns: v.id('_storage'), handler: async (ctx, args) => {
  ...
  if (!upload || owner?.deletionRequested || owner?.tokenIdentifier !== args.tokenIdentifier || upload.expiresAt <= Date.now()) throw new ConvexError('Upload expired.');
...
export const media = internalQuery({ args: { id: v.string(), kind: v.union(v.literal('post'), v.literal('avatar'), v.literal('story')), now: v.optional(v.number()), tokenIdentifier: v.string() }, returns: v.union(v.id('_storage'), v.null()), handler: async (ctx, args) => {
  const me = await ctx.db.query('profiles').withIndex('by_tokenIdentifier', q => q.eq('tokenIdentifier', args.tokenIdentifier)).unique();
  if (!me || me.deletionRequested) return null;
  if (args.kind === 'story') {
    const id = ctx.db.normalizeId('stories', args.id); const story = id && await ctx.db.get('stories', id);
    return story && args.now !== undefined && story.expiresAt > args.now ? story.storageId : null;
  }
