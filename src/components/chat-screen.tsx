...
import { Alert, Keyboard, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, useWindowDimensions, View } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { FeedIcon } from './feed-icon';

export const chatAvatar = require('../../assets/images/chat/avatar.png');
export type ChatMessage = { id: string; text: string; outgoing?: boolean; time: string; image?: ImageSource; read?: boolean; width?: number; status?: 'pending' | 'failed'; retry?: () => void };
...
export function ChatScreen({ onBack, messages, setMessages, peer, onSend, beforeMessages, live = false, onAtBottom }: {
  onBack: () => void; messages: ChatMessage[]; setMessages?: Dispatch<SetStateAction<ChatMessage[]>>;
  peer?: { name: string; avatar: (size: number) => ReactNode }; onSend?: (text: string) => void;
  beforeMessages?: ReactNode; live?: boolean; onAtBottom?: (atBottom: boolean) => void;
}) {
  ...
  useEffect(() => {
    if (!live || !lastId || !nearBottom.current) return;
  ...
  const send = () => {
    ...
    else setMessages?.(current => [...current, { id, text: draft.trim(), outgoing: true, time, image: attachment ? { uri: attachment } : undefined }]);
    setDraft(''); setAttachment(undefined);
  };
  async function pickPhoto() {
    if (live) { Alert.alert('Text messages', 'Photo sharing is available in the local preview only.'); return; }
  ...
  return <KeyboardAvoidingView style={[styles.screen, { paddingTop: insets.top }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    <View style={[styles.header, { height: 62 * v, paddingBottom: 8 * v, paddingHorizontal: fs(12), gap: fs(12) }]}>
      ...
      <View style={{ marginLeft: fs(14) }}>{peer ? peer.avatar(fs(49)) : <Image source={chatAvatar} style={{ width: fs(49), height: fs(49), borderRadius: fs(25) }} />}</View>
      <View style={{ flex: 1, gap: 4 * v }}><Text style={{ color: '#090E29', fontSize: fs(17), fontWeight: '700', letterSpacing: -0.5 }} numberOfLines={1}>{peer?.name ?? 'alex.rivera'}</Text><View style={{ flexDirection: 'row', gap: fs(5), alignItems: 'center' }}>{!live && <View style={{ width: fs(8), height: fs(8), borderRadius: fs(4), backgroundColor: '#18CB2E' }} />}<Text style={{ color: '#8A90A7', fontSize: fs(12) }}>{live ? 'Private conversation' : 'Online'}</Text></View></View>
      <Pressable accessibilityRole="button" accessibilityLabel="Voice call" onPress={() => Alert.alert('Voice calls', 'Calling is not available yet.')} hitSlop={8}><FeedIcon name="phone" size={fs(21)} color="#090E29" /></Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel="Video call" onPress={() => Alert.alert('Video calls', 'Video calling is not available yet.')} hitSlop={8} style={{ marginLeft: fs(8) }}><FeedIcon name="video-call" size={fs(22)} color="#090E29" /></Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel="Chat options" onPress={() => Alert.alert('Chat options', live ? 'Only you and this member can access this conversation. Messages are stored in your private app inbox.' : 'This is a local demo conversation. Online and read indicators are sample data.')} style={{ marginLeft: fs(6) }} hitSlop={8}><Text style={{ color: '#090E29', fontSize: fs(19), fontWeight: '700', letterSpacing: 1 }}>•••</Text></Pressable>
    </View>
    <ScrollView ref={scrollRef} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" showsVerticalScrollIndicator={false} scrollEventThrottle={100} onScroll={event => { const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent; nearBottom.current = contentOffset.y + layoutMeasurement.height >= contentSize.height - 40; onAtBottom?.(nearBottom.current); }} onContentSizeChange={() => { if (shouldScroll.current) { scrollRef.current?.scrollToEnd({ animated: true }); shouldScroll.current = false; } }} maintainVisibleContentPosition={live ? { minIndexForVisible: 1 } : undefined} contentContainerStyle={{ paddingHorizontal: fs(11), paddingBottom: 10 * v }}>
      <Text style={{ color: '#8990A8', fontSize: fs(11), textAlign: 'center', marginTop: 12 * v, marginBottom: 10 * v }}>{live ? 'Messages' : 'Today'}</Text>
      {beforeMessages}
      {messages.map(message => <View key={message.id} style={{ marginBottom: 4.5 * v, alignItems: message.outgoing ? 'flex-end' : 'flex-start' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: fs(10), maxWidth: '100%' }}>
          {!message.outgoing && (peer ? peer.avatar(fs(35)) : <Image source={chatAvatar} style={{ width: fs(35), height: fs(35), borderRadius: fs(18) }} />)}
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: fs(8), marginTop: 4 * v, marginLeft: message.outgoing ? 0 : fs(47), marginRight: message.outgoing ? fs(5) : 0 }}><Text style={{ color: '#8A90A7', fontSize: fs(11), lineHeight: 13 * v }}>{message.status === 'pending' ? 'Sending…' : message.time}</Text>{message.outgoing && !message.status && <FeedIcon name={message.read ? 'double-check' : 'check'} size={fs(16)} color={message.read ? '#087EFF' : '#8A90A7'} />}</View>
        {message.status === 'failed' && <Pressable accessibilityRole="button" onPress={message.retry} style={{ padding: 8 }}><Text style={{ color: '#D93848', fontSize: 12 }}>Not sent · Tap to retry</Text></Pressable>}
      </View>)}
    </ScrollView>
    ...
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: fs(9), paddingHorizontal: fs(11), paddingTop: 7 * v, paddingBottom: keyboardVisible ? 8 : Math.max(Math.min(insets.bottom, 34), 12) }}>
